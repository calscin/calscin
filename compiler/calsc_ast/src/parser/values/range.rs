...
pub fn parse_ast_range(
    ...
    *ind += 1; // [
    ...
    let start_node = parse_ast_value(tokens, ind, true, false, true, ctx)?; // Auto increments
    ...
    *ind += 1;
    ...
    let end_node = parse_ast_value(tokens, ind, true, false, true, ctx)?; // Auto increments
    ...
    *ind += 1; // ]
    ...
    if tokens[*ind].kind == TokenKind::Minus {
        ...
        *ind += 1; // >
        ...
        increment = Some(parse_ast_value(tokens, ind, true, false, true, ctx)?); // Auto increments
    }
