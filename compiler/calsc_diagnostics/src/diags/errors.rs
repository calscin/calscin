...
pub fn build_type_cast_failed<S: DiagnosticSource, T: Display>(
    from: &T,
    into: &T,
    source: &S,
) -> Diagnostic {
    source.make_diagnostic_simple(
        ...
        format!(
            "type casting {} -> {} failed! {} cannot be cast into {}",
            from, into, from, into
        ),
        None,
        vec![],
...
pub fn build_match_already_branch<S: DiagnosticSource, B: Display>(
    branch: &B,
    source: &S,
) -> Diagnostic {
    source.make_diagnostic_simple(
